...
//   qty 0   →  "+"
//   qty >=1 →  ✕ ‹ N ›   (✕ clears the line; plus a 🛒 link to /order if most-recent)
const renderControl = (el: HTMLElement) => {
  ...
  el.innerHTML =
    `<div class="ac-stepper">` +
      `<button type="button" class="ac-btn ac-clear" aria-label="Remove ${label} from order" title="Remove from order">\u2715</button>` +
...
export const initAddButtons = () => {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (target.closest('.ac-clear')) { setLast(null); setQty(id, 0); pop(ctrl); return; } // ✕ removes the line
